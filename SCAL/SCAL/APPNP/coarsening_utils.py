...
def contract_variation_edges(G, A=None, K=10, r=0.5, algorithm="greedy"):
    ...
    nx_graph = nx.DiGraph(G.W)
    # Get the edge list
    edges = np.array(list(nx_graph.edges()))


    weights = np.ones(M)#np.array([subgraph_cost(G, A, edges[:, e]) for e in range(M)])
    # weights = np.zeros(M)
    # for e in range(M):
