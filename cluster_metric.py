...
def conductance(adjacency, clusters):
  ...
  cluster_indices = np.zeros(adjacency.shape[0], dtype=np.bool_)
  for cluster_id in np.unique(clusters):
    cluster_indices[:] = 0
    cluster_indices[np.where(clusters == cluster_id)[0]] = 1
    adj_submatrix = adjacency[cluster_indices, :]
    inter += np.sum(np.array(adj_submatrix[:, cluster_indices]))
    intra += np.sum(np.array(adj_submatrix[:, ~cluster_indices]))
  return intra / (2*inter + intra)
